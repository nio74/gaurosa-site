...
try {
    ...
    if ($piStatus === 'succeeded') {
        ...
        // (Stripe / Klarna / PayPal via Stripe: stesso pattern di bank_transfer)
        // =====================================================
        try {
            ...
            if ($customerId) {
                ...
                error_log("[ConfirmOrder] customer_id #{$customerId} associato all'ordine #{$orderId}");

                $syncOk = syncCustomerToMazGest($customerId);
                error_log("[ConfirmOrder] MazGest sync customer #{$customerId}: " . ($syncOk ? 'OK' : 'FAILED'));
            }
        } catch (Exception $custErr) {
            error_log("[ConfirmOrder] ⚠️ Errore create/sync customer (non bloccante): " . $custErr->getMessage());
        }
